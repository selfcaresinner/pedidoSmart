...
func (s *AdminService) GetGlobalMetrics(w http.ResponseWriter, r *http.Request) {
	query := `SELECT total_transfers, total_cash, total_settled, net_profit, delivered_today, pure_profit, maintenance_fund FROM admin_metrics`
	var transfers, cash, settled, netProfit, pureProfit, maintenanceFund float64
	var delivered int
	err := s.db.Pool.QueryRow(r.Context(), query).Scan(&transfers, &cash, &settled, &netProfit, &delivered, &pureProfit, &maintenanceFund)
	if err != nil {
